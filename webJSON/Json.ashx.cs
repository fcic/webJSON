...
namespace webJSON
{
    ...
    public class Json : IHttpHandler
    {
        ...
        string connStr = "server=192.168.1.254;database=KATES;User ID=Kates_System;Password=Password123";
        SqlDataAdapter da;
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            //string s = context.Request.Form["json"];
            ...
            using (var reader = new System.IO.StreamReader(context.Request.InputStream))
            {
                ...
                if (!string.IsNullOrEmpty(s))
                {
                    ...
                    try
                    {
                        DataTable dt = Newtonsoft.Json.JsonConvert.DeserializeObject<DataTable>(s);
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            try
                            {
                                //context.Response.Write(db.Execute("INSERT INTO [dbo].[BF_User]([UserID],[DepartmentID],[UserName],[TitleName],[CardID],[AllowTimeStartHour],[AllowTimeStartMinute],[AllowTimeEndHour],[AllowTimeEndMinute],[FingerPrint1],[FingerPrint2],[Group1],[Group2],[UserPin],[ValidDate],[IsNative],[InActive],[IsReplicated],[TimeAddNew],[UserAddNewSID],[TimeModifyLast],[TimeModifyLastSID],[UserPWD],[IsLogon],[TimeLastLogout],[GroupSID],[UserPermissionTypeID],[Email],[PhoneMobile],[IsTaOrNot],[EmployeeTypeID])     VALUES('" + dt.Rows[i][0].ToString() + "','99.OTHERS','" + dt.Rows[i][3].ToString() + "','" + dt.Rows[i][2].ToString() + "','" + dt.Rows[i][1].ToString() + "',0,0,23,59,10000,10000,0,0,888888,991212,0,0,0,'2/17/2016  2:12:37',1,null,null,888888,0,null,0,10,null,null,0,null);").ToString());
                                conn.Open();


                                cmd.CommandText = "INSERT INTO [dbo].[BF_User]([UserID],[DepartmentID],[UserName],[TitleName],[CardID],[AllowTimeStartHour],[AllowTimeStartMinute],[AllowTimeEndHour],[AllowTimeEndMinute],[FingerPrint1],[FingerPrint2],[Group1],[Group2],[UserPin],[ValidDate],[IsNative],[InActive],[IsReplicated],[TimeAddNew],[UserAddNewSID],[TimeModifyLast],[TimeModifyLastSID],[UserPWD],[IsLogon],[TimeLastLogout],[GroupSID],[UserPermissionTypeID],[Email],[PhoneMobile],[IsTaOrNot],[EmployeeTypeID])     VALUES('" + dt.Rows[i][0].ToString() + "','99.OTHERS','" + dt.Rows[i][3].ToString() + "','" + dt.Rows[i][2].ToString() + "','" + dt.Rows[i][1].ToString() + "',0,0,23,59,10000,10000,0,0,888888,991212,0,0,0,'2/17/2016  2:12:37',1,null,null,888888,0,null,0,10,null,null,0,null);";
                                cmd.ExecuteNonQuery(); //这样不用每次都开关数据库

                                ds = new DataSet();
                                da = new SqlDataAdapter("select [UserSID] from [KATES].[dbo].[BF_User] where UserID = '"+ dt.Rows[i][0].ToString() + "';", conn);
                                da.Fill(ds, "test");

                                cmd.CommandText = "exec SP_Insert_User_Add_Newuser "+ds.Tables["test"].Rows[0][0].ToString();
                                cmd.ExecuteNonQuery();

                                context.Response.Write(dt.Rows[i][0].ToString()+" , " + dt.Rows[i][1].ToString()+" , " + dt.Rows[i][2].ToString()+" , " + dt.Rows[i][3].ToString()+" is successful imported !"  + "<br/>");
                                ...
                                //File.AppendAllText(".\\log.txt", "error:" + s[i].ToString() + "\r\n" + ex.Message);
    ...
    class DB
    {
        ...
        public int Execute(string sqlStr)
        {
            ...
            //string[] s = sqlStr.Trim(new char[] { ';', ' ' }).Split(';');//因为最后的那个";"后面是空的
        ...
        /// <summary> 
        /// 枚举,标识数据库连接是由DB提供还是由调用者提供 
        /// </summary> 
        private enum SqlConnectionOwnership
        {
            /// <summary>由DB提供连接</summary> 
            Internal,
        ...
        /// <param name="connectionOwnership">标识数据库连接对象是由调用者提供还是由DB提供</param> 
    ...
    /// <summary> 
    /// DBParameterCache提供缓存存储过程参数,并能够在运行时从存储过程中探索参数. 
    /// </summary> 
